...
RSpec.describe 'One Time Payment Test', type: :system do
  ...
  describe 'when payment is successful' do
    context 'when 3d secure required' do
    ...
    context 'when DCC is required' do
      it 'user is redirected to DCC confirmation page, confirms DCC, is redirected to order show view with failed message' do
        visit cart_path
        expect(page).to have_content('Your Cart')
        expect(page).to have_content('This is Product in Cart')
        expect(page).to have_content('This is other Product in Cart')
        ...
        within_frame(find('iframe')) do
          fill_in 'Imię', with: 'Jan'
          fill_in 'Nazwisko', with: 'Kowalski'
          fill_in 'Numer karty', with: '4012888888881881'
          fill_in 'MM', with: '1'
          fill_in 'RR', with: '30'
          fill_in 'CVV', with: '123'
          click_button 'Pay'
  ...
  describe 'when payment failed' do
    ...
    context 'when DCC is required' do
      it 'user is redirected to DCC confirmation page, confirms DCC, is redirected to order show view with success message' do
        visit cart_path
        expect(page).to have_content('Your Cart')
        expect(page).to have_content('This is Product in Cart')
        expect(page).to have_content('This is other Product in Cart')
